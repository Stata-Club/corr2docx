...
program define corr2docx
	...
	qui {
		...
		if `"`note'"' == "" & "`pearson_ignore'" == "" & "`spearman_ignore'" == "" local note = "Lower-triangular cells report Pearson's correlation coefficients, upper-triangular cells are Spearman’s rank correlation"
		else if `"`note'"' == "" & "`pearson_ignore'" == "" local note = "Lower-triangular cells report Pearson's correlation coefficients"
		else if `"`note'"' == "" & "`spearman_ignore'" == "" local note = "Lower-triangular cells report Spearman’s rank correlation"
		
		putdocx table corrtable = (`number', `number'), border(all, nil) border(top) halign(center) note(`"`note'"') layout(autofitw)
